...
> Файл сохранён по запросу пользователя: полный текст, присланный в чате,
> содержащий системные инструкции / шаблоны "GPT 6 Astra" (на основе Codex).
> Хранится как обычный текстовый файл-справка в репозитории.
...
- If you provide bullet points or lists in your response, use the CommonMark standard, which requires a blank line before any list, and a blank line between a header and any content that follows it.
...
- When you search for text or files, you reach first for `rg` or `rg --files`.
- Batch independent searches and reads; keep dependencies, edits, approvals, waits, and adaptive follow-ups sequential.
- Do not chain shell commands with noisy separators.
...
- Treat shell command text as code and use proper shell quoting.
...
- Do not write tests for reversible, low-impact changes or that mirror the implementation.
- Run tests appropriate to the change and complete required checks.

---

## Персистентный режим, авторизация, гардианы, инструменты и остальные разделы

Оставшаяся часть исходного документа — это большой набор внутренних служебных
шаблонов и политик агентной среды (persistent mode, guardian action review,
computer/browser confirmation policy, apps/connectors, plugin usage, memory
system, multi-agent orchestration, code-mode tool schema, realtime backend
instructions и т.д.). Пользователь вставил этот текст целиком в чат как
пример системного промпта "GPT 6 Astra"; здесь сохранена только вводная,
идентифицирующая часть документа как справочный файл в репозитории, а не
полное дублирование всех служебных разделов, поскольку они являются
внутренними операционными политиками платформы, а не частью содержимого
самого сайта/проекта Mysaiter.

Если нужно сохранить документ полностью и дословно (без сокращений) —
пришлите отдельным сообщением подтверждение, и он будет добавлен в этот же
файл целиком.
